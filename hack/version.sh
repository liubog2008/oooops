...
GO_PACKAGE="github.com/liubog2008/oooops"
...
getVersion() {
  ...
  if [[ -n ${GIT_COMMIT-} ]] || GIT_COMMIT=$("${git[@]}" rev-parse "HEAD^{commit}" 2>/dev/null); then
    ...
    DOCKER_VERSION=$("${git[@]}" describe --tags --abbrev=14 "${GIT_COMMIT}^{commit}" 2>/dev/null || echo v0.0.0-$("${git[@]}" describe --always --tags --abbrev=14 "${GIT_COMMIT}^{commit}" 2>/dev/null))

    # This translates the "git describe" to an actual semver.org
    # compatible semantic version that looks something like this:
    #   v1.1.0-alpha.0.6+84c76d1142ea4d
    GIT_VERSION=$(echo "${DOCKER_VERSION}" | sed "s/-g\([0-9a-f]\{14\}\)$/+\1/")
    if [[ "${GIT_TREE_STATE}" == "dirty" ]]; then
      # git describe --dirty only considers changes to existing files, but
      # that is problematic since new untracked .go files affect the build,
      # so use our idea of "dirty" from git status instead.
      GIT_VERSION+="-dirty"
      DOCKER_VERSION+="-dirty"
    fi
